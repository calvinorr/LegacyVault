...
export default function EditBillModal({
  ...
  useEffect(() => {
    if (bill && isOpen) {
      setFormData({
        ...
        accountDetails: {
          billType: bill.accountDetails?.billType || "",
          customerNumber: bill.accountDetails?.customerNumber || "",
          meterNumber: bill.accountDetails?.meterNumber || "",
          tariffType: bill.accountDetails?.tariffType || "",
          paymentMethod: bill.accountDetails?.paymentMethod || "",
          billingFrequency:
            bill.accountDetails?.billingFrequency || "Monthly",
          emergencyNumber: bill.accountDetails?.emergencyNumber || "",
          onlineAccountUrl: bill.accountDetails?.onlineAccountUrl || "",
          category: bill.accountDetails?.category || "bill",
        },
        notes: bill.notes || "",
        confidential:
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateEntry(bill._id, payload);

      onSuccess();
      onClose();
    } catch (err: any) {
      setError(err.message || "Failed to update bill account");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={contentStyle}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={checkboxContainerStyle}>
              ...
              />
              <label
                htmlFor="confidential-bill-edit"
                style={{
                  fontSize: "14px",
                  color: "#374151",
                  cursor: "pointer",
