...
interface EditPensionModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  pension: Entry | null;
...
export default function EditPensionModal({
  isOpen,
  onClose,
  onSuccess,
  pension,
}: EditPensionModalProps) {
  ...
  useEffect(() => {
    if (pension && isOpen) {
      setFormData({
        ...
        accountDetails: {
          pensionType: pension.accountDetails?.pensionType || "",
          policyNumber: pension.accountDetails?.policyNumber || "",
          niNumber: pension.accountDetails?.niNumber || "",
          employerName: pension.accountDetails?.employerName || "",
          contributionRate: pension.accountDetails?.contributionRate || "",
          employerContribution:
            pension.accountDetails?.employerContribution || "",
          annualAllowance: pension.accountDetails?.annualAllowance || "",
          pensionAge: pension.accountDetails?.pensionAge || "",
          transferValue: pension.accountDetails?.transferValue || "",
          onlineAccountUrl: pension.accountDetails?.onlineAccountUrl || "",
          category: pension.accountDetails?.category || "pension",
        },
        notes: pension.notes || "",
        confidential:
          pension.confidential !== undefined ? pension.confidential : true,
        // Categorization fields
        category: (pension as any).category || "Pensions",
        subCategory: (pension as any).subCategory || "",
        supplier: (pension as any).supplier || "",
        tags: (pension as any).tags || [],
      });
      setError(null);
    }
  }, [pension, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateEntry(pension._id, payload);

      onSuccess();
      onClose();
    } catch (err: any) {
      setError(err.message || "Failed to update pension account");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div style={contentStyle}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={checkboxContainerStyle}>
              ...
              />
              <label
                htmlFor="confidential-pension-edit"
                style={{
                  fontSize: "14px",
                  color: "#374151",
                  cursor: "pointer",
