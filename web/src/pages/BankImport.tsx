...
export default function BankImport() {
  ...
  const [transactionData, setTransactionData] = useState<{[sessionId: string]: TransactionData}>({});
  ...
  if (user && user.role !== 'admin') {
    return (
      <div style={{ padding: '32px', textAlign: 'center' }}>
        ...
        <p style={{ color: '#6b7280' }}>This feature is only available to administrators.</p>
      </div>
    );
  }
  ...
  const loadSessions = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to load sessions: ${response.status} ${response.statusText} - ${errorText}`);
      }
        ...
        data.sessions.forEach((session, i) => {
          ...
          if (session.recurring_payments) {
            session.recurring_payments.forEach((suggestion, j) => {
              ...
              if (suggestion.transactions && suggestion.transactions.length > 0) {
                console.log(`    First transaction:`, suggestion.transactions[0]);
              }
            });
          }
        });
  ...
  const loadSessionTransactions = async (sessionId: string) => {
    try {
      ...
      const response = await fetch(`/api/import/sessions/${sessionId}/transactions`, {
        credentials: 'include',
      });
      ...
      const data = await response.json();
      console.log(`Loaded ${data.transaction_count} transactions for session ${sessionId}:`, data);
      ...
      console.error(`Error loading transactions for session ${sessionId}:`, err);
    }
  };
  ...
  const confirmSuggestions = async (sessionId: string, confirmations: any[]) => {
    try {
      const response = await fetch(`/api/import/sessions/${sessionId}/confirm`, {
        method: 'POST',
        credentials: 'include',
        headers: {
      ...
      <>
        <div style={headerStyle}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div>
              <h1 style={{ fontSize: "30px", fontWeight: "bold", color: "#1a1a1a", margin: "0 0 4px 0" }}>
                Bank Import
              </h1>
        ...
        <div style={contentStyle}>
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "200px" }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: "18px", color: "#6b7280", marginBottom: "8px" }}>Loading...</div>
              <div style={{ fontSize: "14px", color: "#9ca3af" }}>Fetching import sessions</div>
            </div>
          </div>
        </div>
      </>
      ...
      <div style={contentStyle}>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "16px", marginBottom: "16px" }}>
            ...
            />
            <button
              style={selectedFile ? primaryButtonStyle : { ...buttonStyle, cursor: 'not-allowed', opacity: 0.5 }}
          ...
          <div style={{
            ...
            <strong>Demo Mode:</strong> Upload a PDF bank statement to automatically detect recurring payments like British Gas, Council Tax, Netflix, and more. The system supports major UK banks including NatWest, Barclays, and HSBC.
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              ...
              <button 
                ...
                onClick={async () => {
                  try {
                    const response = await fetch('/api/import/sessions', { credentials: 'include' });
                    const data = await response.json();
              ...
              {sessions.map((session) => (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                    <div>
                      <h4 style={{ fontSize: "16px", fontWeight: "500", color: "#1a1a1a", margin: "0 0 4px 0" }}>
                        {session.filename}
                      </h4>
                      <p style={{ fontSize: "12px", color: "#6b7280", margin: 0 }}>
                        Uploaded: {new Date(session.createdAt).toLocaleDateString('en-GB')}
                  ...
                  {session.statistics && (
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "12px", marginBottom: "16px" }}>
                      <div style={{ textAlign: "center", padding: "8px", backgroundColor: "#f8fafc", borderRadius: "4px" }}>
                      ...
                      <div style={{ textAlign: "center", padding: "8px", backgroundColor: "#f8fafc", borderRadius: "4px" }}>
                        ...
                        <div style={{ fontSize: "12px", color: "#6b7280" }}>Recurring</div>
                      </div>
                      <div style={{ textAlign: "center", padding: "8px", backgroundColor: "#f8fafc", borderRadius: "4px" }}>
                        <div style={{ fontSize: "18px", fontWeight: "600", color: "#ef4444" }}>
                          {formatCurrency(session.statistics.total_debits)}
                        </div>
                        <div style={{ fontSize: "12px", color: "#6b7280" }}>Debits</div>
                      </div>
                      <div style={{ textAlign: "center", padding: "8px", backgroundColor: "#f8fafc", borderRadius: "4px" }}>
                        <div style={{ fontSize: "18px", fontWeight: "600", color: "#10b981" }}>
                  ...
                  <div style={{ marginBottom: "16px" }}>
                    <h5 style={{ fontSize: "14px", fontWeight: "600", color: "#1a1a1a", marginBottom: "8px" }}>
                      Debug: Raw Session Data
                    </h5>
                    <div style={{ 
                        ...
                        <table style={{ width: "100%", fontSize: "12px", borderCollapse: "collapse" }}>
                          <thead>
                            <tr style={{ backgroundColor: "#f8fafc", borderBottom: "1px solid #e5e7eb" }}>
                              <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: "600", color: "#374151" }}>Date</th>
                              <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: "600", color: "#374151" }}>Description</th>
                              <th style={{ padding: "8px 12px", textAlign: "right", fontWeight: "600", color: "#374151" }}>Amount</th>
                              <th style={{ padding: "8px 12px", textAlign: "left", fontWeight: "600", color: "#374151", fontSize: "11px" }}>Original Text</th>
                            </tr>
                          </thead>
                          <tbody>
                            {transactionData[session._id].transactions
                              ...
                              .map((transaction, txIndex) => (
                                <tr key={txIndex} style={{ borderBottom: "1px solid #f3f4f6" }}>
                                  <td style={{ padding: "6px 12px", color: "#374151" }}>
                                    {new Date(transaction.date).toLocaleDateString('en-GB')}
                                  </td>
                                  <td style={{ padding: "6px 12px", color: "#374151", maxWidth: "200px" }}>
                                    <div style={{ fontSize: "11px", color: "#6b7280", wordWrap: "break-word" }}>
                  ...
                  {session.recurring_payments && session.recurring_payments.length > 0 && (
                    <div>
                      ...
                      <div style={{ display: "grid", gap: "8px" }}>
                        {session.recurring_payments.map((suggestion, index) => (
                          <div
                            key={index}
                            style={{
                              display: "flex",
                              ...
                              {suggestion.status === 'pending' && (
                                <>
                                  <button
                                    ...
                                    onClick={() => confirmSuggestions(session._id, [{ index, action: 'accept' }])}
                                  >
                                    Accept
                                  </button>
                                  <button
                                    style={{ ...buttonStyle, fontSize: "12px", padding: "4px 8px" }}
                                    onClick={() => confirmSuggestions(session._id, [{ index, action: 'reject' }])}
                                  >
                                    Reject
                                  </button>
                                </>
                              ...
                              {suggestion.status !== 'pending' && (
                                <span
                                  style={{
                                    ...
                                    fontSize: "10px",
                                    fontWeight: "500",
                                    backgroundColor: suggestion.status === 'accepted' ? "#10b98120" : "#f59e0b20",
                                    color: suggestion.status === 'accepted' ? "#10b981" : "#f59e0b",
                                  }}
                                >
